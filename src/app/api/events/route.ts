...
重要：name/date/location/description/summaryはすべて中国語（简体中文）に翻訳してください。handleは@英数字のみ。tweet_idは数字のみ（不明なら空文字）。

形式：
{"events":[{"name":"活动名称(中文)","date":"日期","location":"会场地址(中文)","prefecture":"都道府县","handle":"@英数字ユーザー名","account_name":"アカウント名","description":"活动介绍2句话(中文)","free":true/false,"tweet_id":"推文ID数字(不明なら空)"}],"summary":"一句话总结(中文)"}`,
          }
        ],
        temperature: 0.1,
      }),
