...
export async function POST(req: NextRequest) {
  const { keyword, prefecture, period, eventType } = await req.json();
...
X上で実際に宣伝されている本物のイベントのみ掲載してください。8〜12件探してください。`,
        }],
        tools: [{ type: 'x_search' }],
      }),
    });
...
形式：
{"events":[{"name":"イベント名","date":"日程","location":"会場・場所","prefecture":"都道府県","handle":"@ユーザー名","account_name":"アカウント表示名","description":"概要2文","free":true/false,"tweet_id":"ツイートIDの数字のみ（不明な場合は空文字）"}],"summary":"一言まとめ"}`,
          }
        ],
        temperature: 0.1,
      }),
